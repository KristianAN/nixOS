...
let
  ...
  java-debug-plugin = let
    ...
    repository = pkgs.callPackage ./repo.nix {
      inherit src;
      jdk = pkgs.jdk;
    };
  ...
  pkgs.stdenv.mkDerivation {
    ...
    buildPhase = ''
      ...
      ${pkgs.maven}/bin/mvn --offline -Dmaven.repo.local=./repository package
    '';
     ...
     (add-to-list 'eglot-server-programs
             '((java-mode java-ts-mode) .
               ("jdtls"
                ...
                (:bundles ["${java-debug-plugin}/lib/java-debug.jar"]))))
    '';
  };
